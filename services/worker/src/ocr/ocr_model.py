...
GLOBAL_CONFIG_UPSAMPLE=False  #Memory errors at 300, 150 still possibly issues so pause for now, True  #Allow
GLOBAL_CONFIG_FORCE_DOWNSAMPLE_IF_OVER=1000  #ie/ 1000dpi
GLOBAL_CONFIG_FORCE_UPSAMPLE=150 # 300 16+GB memory for 150 page file

def create_tmp_dir(input_pdf):
    # Get the directory of the input PDF and append /tmp/ to it
...
def interface_pdfocr(input_pdf,output_pdf,downsample=300,upsample=0,jobs=2):
    global GLOBAL_CONFIG_DOWNSAMPLE,GLOBAL_CONFIG_UPSAMPLE
    ...
    if dpi and dpi>upsample:
        upsample=0
    ## Echo allow upsample?
    if not GLOBAL_CONFIG_UPSAMPLE:
        print ("[debug] global upsample not active")
    
    if upsample and GLOBAL_CONFIG_UPSAMPLE:
        #[ ] migrate to own function

        # Temp dirs
        tmp_dir = create_tmp_dir(input_pdf)
        base_filename = os.path.basename(input_pdf)
        temp_rasterized = os.path.join(tmp_dir, re.sub(r'\.pdf$', '_rasterized.pdf', base_filename))
        temp_upsampled = os.path.join(tmp_dir, re.sub(r'\.pdf$', '_upsampled.pdf', base_filename))
    
        temp_rasterized=re.sub(r'\.pdf$','_rasterized.pdf',input_pdf)
        ...
        rasterize_pdf(input_pdf, temp_rasterized, dpi=upsample)
        upsample_pdf(temp_rasterized, temp_upsampled, upsample) #gs ghost script won't rasterize & upscale
...
#        a=kkkk
        ...
        ###################################3
        # DOWNSAMPLE [ ] migrate to town function
        # ie 1200 ->> 300
        # optimize==3
        tmp_dir = create_tmp_dir(input_pdf)
        base_filename = os.path.basename(input_pdf)
        temp_downsampled = os.path.join(tmp_dir, re.sub(r'\.pdf$', '_downsampled.pdf', base_filename))
